...
import modeling.bshape_rcnn_heads as bshape_rcnn_heads
...
class Generalized_RCNN(nn.Module):
    def __init__(self):
        ...
        if cfg.MODEL.BSHAPE_ON:
            self.Bshape_Head = get_func(cfg.BSHAPE.ROI_MASK_HEAD)(
                self.RPN.dim_out, self.roi_feature_transform, self.Conv_Body.spatial_scale)
            if getattr(self.Bshape_Head, 'SHARE_RES5', False):
                self.Bshape_Head.share_res5_module(self.Box_Head.res5)
            self.Bshape_Outs = bshape_rcnn_heads.bshape_rcnn_outputs(self.Bshape_Head.dim_out)
    ...
    def _init_modules(self):
        if cfg.MODEL.LOAD_IMAGENET_PRETRAINED_WEIGHTS:
            ...
            if cfg.MODEL.BSHAPE_ON and getattr(self.Bshape_Head, 'SHARE_RES5', False):
                assert compare_state_dict(self.Bshape_Head.res5.state_dict(), self.Box_Head.res5.state_dict())
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        if self.training:
            ...
            if cfg.MODEL.BSHAPE_ON:
                if getattr(self.Bshape_Head, 'SHARE_RES5', False):
                    bshape_feat = self.Bshape_Head(res5_feat, rpn_ret,
                                               roi_has_bshape_int32=rpn_ret['roi_has_bshape_int32'])
                else:
                    bshape_feat = self.Bshape_Head(blob_conv, rpn_ret)
                bshape_pred = self.Bshape_Outs(bshape_feat)
                # return_dict['mask_pred'] = mask_pred
                # mask loss
                loss_bshape = bshape_rcnn_heads.bshape_rcnn_losses(bshape_pred, rpn_ret['bshapes_int32'])
                return_dict['losses']['loss_bshape'] = loss_bshape
