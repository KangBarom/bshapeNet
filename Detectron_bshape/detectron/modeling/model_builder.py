...
import detectron.modeling.bshape_rcnn_heads as bshape_rcnn_heads
...
def generalized_rcnn(model):
    ...
    return build_generic_detection_model(
        ...
        add_roi_bshape_head_func=get_func(cfg.BSHAPE.ROI_MASK_HEAD),
...
def build_generic_detection_model(
    ...
    def _single_gpu_build_func(model):
        ...
        if cfg.MODEL.BSHAPE_ON:
            # Add the mask head
            head_loss_gradients['bshape'] = _add_roi_bshape_head(
                model, add_roi_bshape_head_func, blob_conv, dim_conv,
                spatial_scale_conv
            )
...
def _add_roi_bshape_head(
    model, add_roi_bshape_head_func, blob_in, dim_in, spatial_scale_in
):
    """Add a bshape prediction head to the model."""
    # Capture model graph before adding the mask head
    bbox_net = copy.deepcopy(model.net.Proto())
    # Add the mask head
    blob_bshape_head, dim_bshape_head = add_roi_bshape_head_func(
        model, blob_in, dim_in, spatial_scale_in
    )
    # Add the mask output
    blob_bshape = bshape_rcnn_heads.add_bshape_rcnn_outputs(
        model, blob_bshape_head, dim_bshape_head
    )
    ...
    if not model.train:  # == inference
        ...
        loss_gradients = bshape_rcnn_heads.add_bshape_rcnn_losses(model, blob_bshape)
    return loss_gradients
...
def add_inference_inputs(model):
    ...
    # if cfg.MODEL.BSHAPE_ON:
    #     create_input_blobs_for_net(model.bshape_net.Proto())
