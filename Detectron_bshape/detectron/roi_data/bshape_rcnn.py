...
import detectron.utils.segms_bshape as segm_utils

logger = logging.getLogger(__name__)


def add_bshape_rcnn_blobs(blobs, sampled_boxes, roidb, im_scale, batch_idx):
    """Add Mask R-CNN specific blobs to the input blob dictionary."""
    # Prepare the mask targets by associating one gt mask to each training roi
    # that has a fg (non-bg) class label.
    M = cfg.BSHAPE.RESOLUTION
    ...
    if fg_inds.shape[0] > 0:
        ...
        masks = blob_utils.zeros((fg_inds.shape[0], M**2), int32=False)
        ...
        for i in range(rois_fg.shape[0]):
            ...
            mask = np.array(mask, dtype=np.float32)  # Ensure it's binary np.int32-> np.float32
    ...
    blobs['bshape_rois'] = rois_fg
    blobs['roi_has_bshape_int32'] = roi_has_mask
    blobs['bshapes_int32'] = masks
